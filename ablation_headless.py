...
def run_model_A():
    ...
    state['R_hist'] = []
    # Simulate model A for steps, as in meta version, but without meta-optimizer
    for step in range(steps):
        dx = -state['x'] + bistable_layer(state['x'], alpha, theta_eff) + eps * state['ws'] + 0.03 * torch.randn(n_layers, device=device)
        state['x'] += dt * dx
...
def run_model_A_meta(lambda_decouple=1.0):
    ...
    for step in range(steps):
        ...
        reward = entropy + r
        # Pad feature vector to 68 (4 + 64 zeros) for MetaTunerNN
        features = [entropy, r, lyap, complexity] + [0.0]*64
        experience_buffer.append((features, [alpha_norm, eps_norm], reward))
...
def run_model_B():
    ...
    state['ws'] = torch.tensor(0.0, device=device)
    state['R_hist'] = []
    # Simulate model B for steps, as in meta version, but without meta-optimizer
    for step in range(steps):
